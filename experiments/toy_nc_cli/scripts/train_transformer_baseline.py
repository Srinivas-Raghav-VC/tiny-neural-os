...
from src.transformer_model import (
    TransformerBaseline,
    TransformerConfig,
    action_kind_breakdown_transformer,
...
def run_experiment(
    ...
    model = TransformerBaseline(
        ...
        cols=config.cols,
        families=families,
        config=TransformerConfig(epochs=40, hidden_dim=256, num_heads=4, num_layers=3, context_width=32, seed=seed),
        condition_level=condition_level,
    )
    losses = model.fit(train_eps)
    metrics = evaluate_transformer(model, test_eps)
    breakdown = action_kind_breakdown_transformer(model, test_eps)
...
def main() -> int:
    ...
    results = {
        "standard_family": run_experiment("standard_family", "family", seed=201),
        "standard_command": run_experiment("standard_command", "command", seed=202),
    ...
    out_path = out_dir / "transformer_results.json"
