...
def get_mamba_block(d_model: int, cfg: MambaConfig):
    try:
        from mamba_ssm import Mamba2
        return Mamba2(d_model=d_model, d_state=cfg.d_state, d_conv=cfg.d_conv, expand=cfg.expand)
    except Exception:
        try:
            from mamba_ssm import Mamba
            return Mamba(d_model=d_model, d_state=cfg.d_state, d_conv=cfg.d_conv, expand=cfg.expand)
        except Exception as e:
            raise RuntimeError("mamba_ssm is not installed or importable") from e


class ResidualMambaBlock(nn.Module):
    def __init__(self, d_model: int, cfg: MambaConfig):
        super().__init__()
        self.norm1 = nn.LayerNorm(d_model)
        self.mamba = get_mamba_block(d_model, cfg)
...
class ScreenMamba(nn.Module):
    def __init__(self, rows: int, cols: int, num_families: int, config: MambaConfig, condition_level: ConditionLevel):
        super().__init__()
        self.rows = rows
        self.cols = cols
        self.config = config
        ...
        self.layers = nn.ModuleList([ResidualMambaBlock(config.hidden_dim, config) for _ in range(config.num_layers)])
    ...
    def forward(self, batch: dict) -> torch.Tensor:
        ...
        for layer in self.layers:
            x = layer(x)
        x = self.out_norm(x)
        return self.decoder(x).reshape(x.shape[0], x.shape[1], self.rows, self.cols, self.vocab_size)


class MambaBaseline:
    def __init__(self, rows: int, cols: int, families: list[str], config: MambaConfig, condition_level: ConditionLevel):
        self.rows = rows
        self.cols = cols
        self.family_to_idx = {name: i for i, name in enumerate(sorted(set(families)))}
        self.config = config
        self.condition_level = condition_level
        self.model = ScreenMamba(rows, cols, len(self.family_to_idx), config, condition_level).to(config.device)
...
def evaluate_mamba(model: MambaBaseline, episodes: list[Episode]) -> dict[str, float]:
    vals = [summarize_rollout(model.rollout(ep), ep) for ep in episodes]
    return {k: float(np.mean([v[k] for v in vals])) for k in vals[0]}


def action_kind_breakdown_mamba(model: MambaBaseline, episodes: list[Episode]) -> dict[str, dict[str, float]]:
